...
def word_mover_score(refs, hyps, idf_dict_ref, idf_dict_hyp, stop_words='nltk', 
...
# new for tls
def get_timeline_sent_list(timeline):
	sent_list = []

	for date in sorted(timeline.dates_to_summaries):
		sent_list.extend(timeline[date])
...
def get_wordmover_score(timeline, ground_truth, wm_stop_words='nltk', device='cpu'):
	...
	timeline_sents = get_timeline_sent_list(timeline)
	gt_sents = get_timeline_sent_list(ground_truth)

	full_timeline_text = ' '.join(timeline_sents)
	full_gt_text = ' '.join(gt_sents)

	# WordMover original by sent idf
	idf_dict_hyp = get_idf_dict(timeline_sents)
	idf_dict_ref = get_idf_dict(gt_sents)
	...
	while (len(gt_inds) > 0 and len(tl_inds) > 0):
		...
		top_gt_ind = gt_inds[top_score_ind]
		top_tl_ind = tl_inds[top_score_ind]

		keep_mask = (gt_inds != top_gt_ind) & (tl_inds != top_tl_ind)
	...
	return {
		...
		'aligned wordmover avg': np.mean(aligned_scores),
		'aligned wordmover median': np.median(aligned_scores),
		'aligned wordmover std': np.std(aligned_scores)
	}
